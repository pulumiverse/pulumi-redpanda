...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `redpanda` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `videmsky` with the `ORG` of your provider.
   - replace all instances of the `github.com/videmsky/pulumi-redpanda` repository with the `REPOSITORY` location
...
- binary: pulumi-resource-redpanda
  ...
  ignore: []
  ldflags:
  - -X github.com/videmsky/pulumi-redpanda/provider/pkg/version.Version={{.Tag}}
...
├── config.yaml
├── metadata.json
├── pulumi-redpanda_darwin_amd64_v1
│   └── pulumi-resource-redpanda
├── pulumi-redpanda_darwin_arm64
│   └── pulumi-resource-redpanda
├── pulumi-redpanda_linux_amd64_v1
│   └── pulumi-resource-redpanda
├── pulumi-redpanda_linux_arm64
│   └── pulumi-resource-redpanda
├── pulumi-redpanda_windows_amd64_v1
│   └── pulumi-resource-redpanda.exe
└── pulumi-redpanda_windows_arm64
    └── pulumi-resource-redpanda.exe
```
